...
class Twocheckout extends NonmerchantGateway
{
     ...
     */
    private static $authors = [['name' => 'Nelsahost.doo', 'url' => 'http://www.nelsahost.com']];
    /**
    ...
    public function __construct()
    {
        ...
        Language::loadLang('twocheckout', null, dirname(__FILE__) . DS . 'language' . DS);
    }
    ...
    {
        return Language::_('Twocheckout.name', true);
    }
        ...
        $rules = [
            'vendor_id' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Twocheckout.!error.vendor_id.empty', true)
                ]
            ],
            'secret_word' => [
                'empty' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Twocheckout.!error.secret_word.empty', true)
                ]
            ],
            'test_mode'=>[
                'valid'=>[
                    ...
                    'message'=>Language::_('Twocheckout.!error.test_mode.valid', true)
                ]
            ]
        ];
    ...
    public function buildProcess(array $contact_info, $amount, array $invoice_amounts = null, array $options = null)
    {
        ...
        $fields = [
            ...
            'x_Receipt_Link_URL' => Configure::get('Blesta.gw_callback_url')
                . Configure::get('Blesta.company_id') . '/Twocheckout/',
        ...
        $rules = [
            'key' => [
                'valid' => [
                    ...
                    'message' => Language::_('Twocheckout.!error.key.valid', true)
                ]
            ],
            'credit_card_processed' => [
                'completed' => [
                    'rule' => ['compares', '==', 'Y'],
                    'message' => Language::_('Twocheckout.!error.credit_card_processed.completed', true)
                ]
            ],
            'sid' => [
                'valid' => [
                    'rule' => ['compares', '==', $this->ifSet($this->meta['vendor_id'])],
                    'message' => Language::_('Twocheckout.!error.sid.valid', true)
                ]
            ]
        ];
    ...
    private function sendApi($post_to, array $params)
    {
        ...
        $this->Http->setHeader(
            'Authorization: Basic '
            . base64_encode($this->ifSet($this->meta['api_username']) . ':' . $this->ifSet($this->meta['api_password']))
        );
        $response = $this->Http->post($post_to, $params);

        // Log data sent
        $this->log($post_to, serialize($this->maskData($params, ['username', 'password'])), 'input', true);
